-- إعداد نظام نقاط الترتيب الكامل

-- التأكد من وجود جدول نقاط الترتيب
CREATE TABLE IF NOT EXISTS public.student_ranking_points (
  ...
  behavior_perfect boolean DEFAULT false, -- أدب كامل طوال الشهر
  
  -- نقاط الغياب
  absence_points numeric DEFAULT 0,
  absence_count integer DEFAULT 0,
  perfect_attendance boolean DEFAULT false, -- لا غياب طوال الشهر
  
  -- نقاط المذاكرة الشهرية
  monthly_review_points numeric DEFAULT 0,
  monthly_review_score numeric, -- العلامة من 100
  
  -- نقاط الاختبارات
  exam_points numeric DEFAULT 0,
  exam_count integer DEFAULT 0, -- عدد الاختبارات (غير الإعادة)
...
-- تفعيل RLS
ALTER TABLE public.student_ranking_points ENABLE ROW LEVEL SECURITY;

-- سياسة للأدمن فقط
DROP POLICY IF EXISTS "Allow all operations on student_ranking_points" ON public.student_ranking_points;

CREATE POLICY "Admin only access to ranking points"
ON public.student_ranking_points
FOR ALL
USING (true); -- سنتحكم بالعرض من التطبيق

-- إنشاء فهارس
CREATE INDEX IF NOT EXISTS idx_ranking_points_student_month ON public.student_ranking_points(student_id, month, year);
CREATE INDEX IF NOT EXISTS idx_ranking_points_month_year ON public.student_ranking_points(month, year);
CREATE INDEX IF NOT EXISTS idx_ranking_points_total ON public.student_ranking_points(total_points DESC);

-- trigger للتحديث التلقائي
CREATE TRIGGER update_student_ranking_points_updated_at
BEFORE UPDATE ON public.student_ranking_points
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();

-- دالة لحساب نقاط الترتيب الشهرية
...
-- دالة لحساب نقاط الترتيب لجميع الطلاب في شهر معين
